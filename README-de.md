**Available Languages:** [English](README.md) | [Español](README-es.md) | Deutsch | [Français](README-fr.md) | [日本語](README-jp.md) | [Português](README-pt.md)
...
Erstellen Sie Kontextdateien, die Tools dauerhaft über die Struktur, Standards und Präferenzen Ihres Projekts informieren.
...
### Zuerst Planen Bitten

Bitten Sie den Assistenten, Schritte, Risiken und schnelle Tests zu skizzieren, bevor er Code anfasst, damit Sie den Ansatz überprüfen und anpassen können.
...
### Mit Hochleistungsmodell Planen

Beim Sammeln von Anforderungen oder Entwerfen von Spezifikationen vorübergehend zu einem leistungsfähigeren Modell oder erweiterten Reasoning-Modus wechseln, damit es lesen, synthetisieren und einen Plan vorschlagen kann, bevor es codiert.
...
Wählen Sie bewusst gut etablierte Bibliotheken mit guter Stabilität, die vor den Trainingscut off-Daten des Modells existierten, für bessere LLM-unterstützte Code-Generierung.
...
Fügen Sie Screenshots ein und iterieren Sie - machen Sie einen Screenshot vom Ergebnis, vergleichen, wiederholen.
...
### Nach ASCII-Wireframes Fragen

Beim Verfeinern von Layouts lassen Sie den Assistenten ASCII-Wireframes skizzieren, damit Sie Hierarchie und Abstände bewerten können, bevor Sie CSS anfassen.
...
Erstellen Sie Projekte durch Konversation statt durch traditionelles Codieren - sprechen, Änderungen akzeptieren und iterieren, bis es funktioniert.
...
### Verständnis Vor dem Codieren Bestätigen

Bitten Sie das Tool explizit, sein Verständnis der Aufgabe zu bestätigen, bevor Sie mit der Implementierung beginnen, um Ausrichtung sicherzustellen und fehlausgerichtete Erwartungen zu reduzieren.
...
### Code Supereinfach Halten

Schreiben Sie einfachen Code mit klaren Funktionsnamen, vermeiden Sie Vererbung und clevere Tricks - einfacher Code funktioniert besser mit Assistenten.
...
### Mit Bestehendem Code Vorbereiten

Beginnen Sie, indem Sie bestehenden Code in den Chat werfen, um den Kontext zu setzen, dann modifizieren Sie von dort aus.
...
### Kontext für Neue Bibliotheken Bereitstellen

Wenn Sie Bibliotheken außerhalb der Trainingsdaten des Modells verwenden, geben Sie ihm aktuelle Beispiele und Dokumentation, um ihm beizubringen, wie die Bibliothek funktioniert.
...
Schreiben Sie benutzerdefinierten Code, anstatt mehr Bibliotheken hinzuzufügen, wenn Sie mit Assistenten arbeiten.
...
### Alles für Assistenten-Debugging Protokollieren

Entwerfen Sie Systeme mit umfassendem Logging, damit Agenten Logs lesen können, um zu verstehen, was passiert, und Probleme selbst zu diagnostizieren.
...
### Selbst auf Assistenten-Ausgabe Iterieren

Nachdem der Assistent die Arbeit abgeschlossen hat, manuell iterieren und die Implementierung verfeinern, anstatt sie so zu akzeptieren.
...
### Assistenten-Ausgabestile Wechseln

Wählen Sie den Assistenten-Ausgabestil, der zu Ihrem aktuellen Ziel passt.
...
### Tools nach Gesprächsstil Wählen

Wählen Sie Coding-Assistenten basierend darauf, ob Sie menschenähnliche Zusammenarbeit oder strukturierte, roboterartige Effizienz bevorzugen - Gesprächspersönlichkeit beeinflusst Produktivität und Freude erheblich.
...
Bevor Sie eine neue Aufgabe beginnen, wählen Sie zwei Hebel: das richtige Modell (Modalität, Kontextlänge, Tool-Calling-Zuverlässigkeit, Latenz, Kosten) und die richtige Reasoning-Stufe (mehr/weniger Thinking-Tokens zuweisen) — verwenden Sie nicht blind die Standardeinstellung.
...
### Speicherdateien Zentralisieren

Führen Sie ein kanonisches Anweisungsdokument und leiten Sie alle anderen Agentendateien mit einer auffälligen Zeiger-Zeile, einem Symlink oder einem @file-Include darauf, damit die toolübergreifende Anleitung konsistent bleibt.
...
Lassen Sie den Assistenten nicht zu weit auf dem falschen Weg gehen - unterbrechen Sie, geben Sie Feedback und leiten Sie um, sobald Sie Probleme bemerken.
...
### Einen Agenten als Coding-Partner Nutzen

Arbeiten Sie zusammen wie mit einem Coding-Partner - erklären Sie Probleme, holen Sie Feedback ein und arbeiten Sie gemeinsam an Lösungen.
...
### Davon Lernen, Selbst Codieren

Nutzen Sie Assistenten, um neue Sprachen und Konzepte zu lernen, dann wenden Sie dieses Wissen beim Codieren an.
...
### Schnelle, Narrensichere Tools Erstellen

Erstellen Sie Tools, die schnell reagieren, klare Fehlermeldungen liefern und sich gegen Missbrauch durch Agenten schützen.
...
Hören Sie auf zu warten, bis ein Agent fertig ist, bevor Sie einen anderen starten - führen Sie mehrere Agenten parallel an separaten Features ohne Konflikte oder Verwirrung aus.
...
### Eine Sitzung Sollte Ein Ziel Haben

Verwenden Sie den Prompt `Das Ziel dieser Sitzung ist <spezifisches Ziel>. Informieren Sie mich, wenn wir vom Kurs abkommen.` entweder zu Beginn jeder Sitzung oder fügen Sie ihn zu Ihrer Speicherdatei (AGENTS.md, CLAUDE.md) hinzu, um Kontextvergiftung zu verhindern und die Steuerbarkeit des Agenten zu erhöhen - Anwendung des Single-Responsibility-Prinzips auf KI-Konversationen.
...
### Feature-Sitzungen Verwenden

Isolieren Sie jedes Feature oder jede Aufgabe in separaten Sitzungen, um Kontext-Bloat zu reduzieren und die Genauigkeit zu verbessern, genau wie Feature-Branches in Git Code-Änderungen isolieren.
...
### Günstig und schnell starten; eskalieren wenn festgefahren

Beginnen Sie mit schnelleren/günstigeren Modellen für Routineaufgaben, dann eskalieren Sie zu leistungsstärkeren Modellen nur wenn Sie auf komplexe Probleme stoßen.
...
### Rollback-Punkte Beim Codieren Erstellen

Erstellen Sie Checkpoints, zu denen Sie zurückkehren können, wenn Experimente fehlschlagen—erfassen Sie bekannte gute Arbeitszustände vor riskanten Änderungen.
